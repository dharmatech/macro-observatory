...
def test_render_storage_report_uses_fixed_kb_columns_and_totals(tmp_path: Path) -> None:
    ...
    size_fields = [line[18:28] for line in data_rows]
    assert all(len(size_field) == 10 for size_field in size_fields)
    assert any(size_field.strip() == "1,352.1" for size_field in size_fields)
